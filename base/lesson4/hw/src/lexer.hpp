...
private: 
    ...
    std::unordered_map<std::string, FactoryMethod> factory = {
        ...
        {"else", [this]() { return std::make_unique<TokenNoVal>(Type::ELSE, line); } },
...
public:
    ...
    void tokenize() {
        while (current!=eof) {
            ...
            else throw CompilerError("Unexpected character: '" + std::string(1, *current) +"' on line " + std::to_string(line));
        }
...
private:
    ...
    void handleSymbol() {
        ...
        auto it = factory.find(res);
        if (it != factory.end()) tokens.push_back(it->second()); 
        else throw CompilerError("Unexpected operator: '" + res + "' on line " + std::to_string(line));
    }
