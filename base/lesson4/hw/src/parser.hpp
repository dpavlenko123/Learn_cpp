...
class Parser {
    ...
    void expected(Type t) {
        if (!match(t)) throw CompilerError("Expected" + typeToString(t) + " on line " + getLine());
    }
public:
...
private:
    ...
    node_ptr statementList() {
        ...
        if (stmtList.empty()) throw CompilerError("Empty program");
    ...
    node_ptr statement() {
        ...
        if (op==Type::IF) {
            ...
            expected(Type::RPAREN);
            auto ifBody = block();
            node_ptr elseBody = nullptr;
            if (type()==Type::ELSE) {
                next();
                elseBody = block();
            }
            return construct<IfNode>(std::move(cond), std::move(ifBody), std::move(elseBody));
        }
        if (op==Type::PRINT) {
            next();
            expected(Type::LPAREN);
    ...
    node_ptr assignment() {
        ...
        if (type()==Type::ASSIGN) {
            ...
            else throw CompilerError("Expected IDENTIFIER before '=' on line" + getLine());
        }
        return left;
    }
    ...
    node_ptr factor() {
        ...
        if (type()==Type::LPAREN) {
            ...
            if (!match(Type::RPAREN)) throw CompilerError("Expected ')' on line " + getLine());
            return node;
        }
        throw CompilerError("Unexpected symbol on line" + getLine());
    }
};
