...
protected:
    ...
    node_ptr make_idnty(std::string name) {
        return std::make_unique<Node>(std::in_place_type<IdntyNode>, std::move(name));
    }

    node_ptr make_number(double value) {
        return std::make_unique<Node>(std::in_place_type<NumberNode>, value);
    }

    node_ptr make_binary(node_ptr left, node_ptr right, std::string op) {
        return std::make_unique<Node>(std::in_place_type<BinaryNode>, std::move(left), std::move(right), std::move(op));
    }
};
...
TEST_F(ParserTest, CompareTrees) {
    ...
    auto tree = make_binary(
        make_idnty("x"),
        make_binary(make_number(2), make_number(3), "+"),
        "="
    );
